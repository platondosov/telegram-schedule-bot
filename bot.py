...
# Словарь для хранения выбранной недели пользователями
# Формат: {user_id: "I" или "II" или "auto"}
user_selected_weeks = {}
...
def save_data():
    ...
    try:
        with open(DATA_FILE, 'wb') as f:
            pickle.dump(user_selected_weeks, f)
...
def load_data():
    ...
    global user_selected_weeks
    try:
        with open(DATA_FILE, 'rb') as f:
            user_selected_weeks = pickle.load(f)
        print(f"✅ Данные загружены. Пользователей: {len(user_selected_weeks)}")
...
# Расписание для вашей группы (ваш код остается без изменений)
schedule = {
    "Понедельник": {
...
3. Используйте кнопку "🔄 Сменить неделю" для переключения между I и II неделями
...
def week_command(message):
    ...
    user_week = get_user_week(message.chat.id)
    today = datetime.now()
    week_num = (today - START_DATE).days // 7 + 1 if today >= START_DATE else 0
...
*Режим:* {"Ручной" if message.chat.id in user_selected_weeks and user_selected_weeks[message.chat.id] != "auto" else "Автоматический"}
"""
...
def show_day_with_week_buttons(message, day_name, prefix=""):
    ...
    if day_name in schedule and user_week in schedule[day_name]:
        response = f"{prefix}\n\n"
        response += schedule[day_name][user_week]
        ...
        mode_text = "Ручной режим" if user_id in user_selected_weeks and user_selected_weeks[user_id] != "auto" else "Автоматический режим"
...
def show_week_selection_menu(message):
    ...
    current_mode = "Автоматический" if user_id not in user_selected_weeks or user_selected_weeks[user_id] == "auto" else "Ручной"
