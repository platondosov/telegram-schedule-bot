...
# ================ РАСПИСАНИЕ ================
# ================ РАСПИСАНИЕ (ЗАГРУЗКА ИЗ ФАЙЛА) ================
def load_schedule():
    try:
        with open('schedule.json', 'r', encoding='utf-8') as f:
            # Преобразуем ключи-строки в числа (т.к. JSON ключи всегда строки, а у нас подгруппы 1 и 2)
...
def handle_contact(message):
    if message.contact is not None:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT OR REPLACE INTO users (user_id, first_name, last_name, username, phone_number)
            VALUES (?, ?, ?, ?, ?)
        ...
        bot.send_message(
            user_id, 
            "✅ Отлично, контакт сохранен! Теперь давайте настроим расписание.", 
...
def handle_commands(message):
    ...
    if not is_user_registered(user_id) and cmd != 'help':
        bot.send_message(
            user_id, 
            "👋 Добро пожаловать!\nДля использования бота необходимо зарегистрироваться.\nНажмите кнопку *📱 Поделиться контактом* внизу экрана.", 
...
def handle_text(message):
    ...
    if not is_user_registered(user_id):
        bot.send_message(user_id, "Сначала отправьте контакт для регистрации 📱", reply_markup=get_registration_keyboard())
        return
